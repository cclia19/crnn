...
if __name__ == "__main__":
    ...
    for epoch in range(params.nepoch):
        ...
        while i < len(train_loader):
            ...
            #-----------------------------------------do checkpointing---------------------------------------------------
            #save all the trained model path
            if i % params.saveInterval == 0:
                torch.save(crnn.state_dict(), '{0}/netCRNN_{1}_{2}.pth'.format(params.expr_dir, epoch, i))
                if accuracy > best_valAccuracy:
                    best_valAccuracy = accuracy
                    print('accuracy improved to %f, saving model to netCRNN_%d_%d.pth\n' % (best_valAccuracy, epoch, i))
                else:    
                    print('accuracy did not improve from %f \n' % (best_valAccuracy))
            
            #only save the trained model path with improved accuracy
            """if i % params.saveInterval == 0:
                if accuracy > best_valAccuracy:
                    best_valAccuracy = accuracy
                    print('accuracy improved to %f, saving model to netCRNN_%d_%d.pth\n' % (best_valAccuracy, epoch, i))
